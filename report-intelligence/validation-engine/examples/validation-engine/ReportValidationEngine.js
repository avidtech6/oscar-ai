...
exports.ReportValidationEngine = void 0;
...
class ReportValidationEngine {
     ...
     */
    async validate(schemaMappingResult) {
        const startTime = Date.now();
        const validationResultId = (0, ValidationResult_1.generateValidationResultId)();
        try {
            // Create initial validation result
            const validationResult = {
                ...(0, ValidationResult_1.createValidationResult)(schemaMappingResult.id, schemaMappingResult.decompiledReportId, schemaMappingResult.reportTypeId),
            ...
            validationResult.scores.overallScore = (0, ValidationResult_1.calculateOverallScore)(validationResult.scores);
     ...
     */
    getApplicableRules(reportTypeId) {
        return this.validationRules.filter(rule => {
            if (!rule.enabled)
                return false;
            if (rule.appliesTo.includes('all'))
                return true;
            if (reportTypeId && rule.appliesTo.includes(reportTypeId))
                return true;
            return false;
        });
    }
     ...
     */
    async processRule(rule, schemaMappingResult, validationResult) {
        ...
        switch (rule.type) {
            case 'compliance':
                ...
                if (!passed) {
                    violation = {
                        id: (0, ValidationResult_1.generateComplianceViolationId)(),
            ...
            case 'quality':
                ...
                if (!passed) {
                    qualityIssue = {
                        id: (0, ValidationResult_1.generateQualityIssueId)(),
        ...
        if (!passed) {
            finding = {
                id: (0, ValidationResult_1.generateValidationFindingId)(),
                ruleId: rule.id,
                ruleName: rule.name,
                ruleType: rule.type,
                severity: rule.severity,
     ...
     */
    evaluateComplianceRule(rule, schemaMappingResult) {
     ...
     */
    evaluateQualityRule(rule, schemaMappingResult) {
        // Simplified quality checking
        // Check section clarity
        if (rule.id === 'qual_001') {
            // Check if sections have clear titles
            const unclearSections = schemaMappingResult.mappedFields.filter(field => field.fieldName.length < 3 || field.fieldName === 'Untitled');
     ...
     */
    evaluateCompletenessRule(rule, schemaMappingResult) {
     ...
     */
    evaluateConsistencyRule(rule, schemaMappingResult) {
        // Check date consistency
        if (rule.id === 'cons_001') {
            // Simplified check - in real system, would parse and compare dates
            return schemaMappingResult.schemaGaps.filter(gap => gap.type === 'mismatched_schema').length === 0;
        }
        return true; // Default to passing
    }
    /**
     ...
     */
    evaluateTerminologyRule(rule, schemaMappingResult) {
     ...
     */
    calculateScores(validationResult, applicableRules, schemaMappingResult) {
        // Initialize scores
        const scores = {
            complianceScore: 0,
            qualityScore: 0,
        ...
        if (totalRules > 0) {
            ...
            for (const ruleType of Object.keys(rulesByType)) {
        ...
        if (applicableRules.length > 0) {
            ...
            scores.consistencyScore = Math.max(0, 90 - (validationResult.findings.filter(f => f.ruleType === 'consistency').length * 15));
        }
        return scores;
    }
    /**
     ...
     */
    createSchemaMappingSnapshot(schemaMappingResult) {
        return {
     ...
     */
    createReportTypeSnapshot(reportType) {
        return {
            id: reportType.id,
...
exports.ReportValidationEngine = ReportValidationEngine;
